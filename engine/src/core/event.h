...
namespace project {
  ...
  struct event_instance {
    PROJECT_INLINE virtual ~event_instance() = default;
  };
  ...
  struct callback_instance {
    PROJECT_INLINE virtual ~callback_instance() = default;
    virtual bool run(event_instance *) = 0;
  };

  template <typename T>
  struct event_callback : callback_instance {
    PROJECT_INLINE event_callback(callback_function<T>& clbk): callback(clbk) {}

    PROJECT_INLINE bool run(event_instance* e) { 
