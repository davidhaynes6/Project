...
namespace project {
  struct dispatcher {
    ...
    PROJECT_INLINE void post(Args&&...args) {
      PROJECT_STATIC_ASSERT(std::is_base_of<event_instance, T>::value);
      _events[type_id<T>()] = new T(std::forward<Args>(args)...);
    }

    // registers a callback for the event of type T
    template <typename T>
    PROJECT_INLINE void add_callback(callback_function<T>&& clbk) {
      PROJECT_STATIC_ASSERT(std::is_base_of<event_instance, T>::value);
