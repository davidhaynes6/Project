...
namespace project {
  struct script_props {
    PROJECT_INLINE script_props(ecs::entityid e, ecs::registry* r, asset_registry* a):
  ...
  struct script_instance {
    PROJECT_INLINE virtual ~script_instance() = default;

    PROJECT_INLINE void init(const script_props& props) {
      this->_registry = props.registry;
      this->_assets = props.assets;
      this->_entity = props.entity;
    }

    PROJECT_INLINE void virtual on_collision(ecs::entity) {}
    PROJECT_INLINE void virtual on_update(float) {}
    PROJECT_INLINE void virtual on_destroy() {}
    PROJECT_INLINE void virtual on_start() {}
  ...
  protected:
    ...
    PROJECT_INLINE ecs::entity add_entity() {
    ...
    PROJECT_INLINE void destroy() {
      _registry->destroy(_entity);
    }

    PROJECT_INLINE ecs::entity find_entity(const std::string& name) {
    ...
    template<typename T>
    PROJECT_INLINE T* get_asset(const std::string& name) {
      return _assets->get<T>(name);
    }

PROJECT_INLINE void play_audio(std::string name, int channel = -1, int loops = 0, int vol = 50) {
