...
namespace project::ecs
{
    struct registry
    {
        ...
        PROJECT_INLINE entityid add_entity()
        {
        ...
        PROJECT_INLINE void destroy(entityid e)
        {
            for (auto &[_, a] : _components)
            {
                a->erase(e);
        ...
        PROJECT_INLINE bool is_alive(entityid e)
        {
            return _signatures.count(e);
        }

        template <typename T>
        PROJECT_INLINE entitylist view()
        {
            entitylist list;
        ...
        template <typename T>
        PROJECT_INLINE T &get_component(entityid e)
        {
            PROJECT_ASSERT(_signatures.count(e) && "out of range!");
            return get_component_array<T>()->get(e);
        }

        template <typename T, typename... Args>
        PROJECT_INLINE T &add_component(entityid e, Args &&...args)
        {
            PROJECT_ASSERT(_signatures.count(e) && "out of range!");
        ...
        template <typename T>
        PROJECT_INLINE void remove_component(entityid e)
        {
        ...
        template <typename T>
        PROJECT_INLINE bool has_component(entityid e)
        {
        ...
        template <typename T>
        PROJECT_INLINE component_array<T> *get_component_array()
        {
