...
namespace project::ecs {
  struct scene {
    ...
    PROJECT_INLINE ~scene() {
      for (auto& s : _systems) { PROJECT_DELETE(s); }
      _registry.clear();
      _systems.clear();
    }
    ...
    PROJECT_INLINE void serialize(const std::string& path) {
    ...
    PROJECT_INLINE bool deserialize(const std::string& path) {
      YAML::Node root;
      try {
        root = YAML::LoadFile(path);
      }
      catch (YAML::ParserException e) {
        PROJECT_INFO("failed to deserialize scene!");
        return false;
      }  
